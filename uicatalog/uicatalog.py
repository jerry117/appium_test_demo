...
class SimpleIOSTests(unittest.TestCase):
    ...
    def setUp(self):
        # set up appium
        app = os.path.abspath('/Users/jerryli/Documents/uicatalog/ios-uicatalog/build/Release-iphonesimulator/UICatalog.app')
    ...
    def test_ui_computation(self):
        # Buttons = self.driver.find_element_by_xpath('//XCUIElementTypeStaticText[@name="Buttons"]')
        Buttons = self.driver.find_element_by_ios_predicate('label == "Buttons"')
        Buttons.click()

        # 用显示等待来等待某个元素的出现，当作断言
        WebDriverWait(self.driver, 5).until(
            lambda x: x.find_element_by_xpath('(//XCUIElementTypeButton[@name="Button"])[1]'), "Find element fail! Button Not Found!"
        )
